...
-- MAGIC %md
-- MAGIC # Dataset:
-- MAGIC 1. Turbine metadata: Turbine ID, location (1 row per turbine)
-- MAGIC 1. Turbine status: Historical turbine status based to analyse which part is faulty 
-- MAGIC 1. Turbine sensor stream: Realtime streaming flow from wind turbine sensor (vibration, energy produced, speed etc)
...
-- DBTITLE 1,DLT Table for Turbine Data
...
-- DBTITLE 1,DLT Table for Turbine Sensor Data
...
-- DBTITLE 1,DLT Table for Turbine Historical Status
...
-- MAGIC %md-sandbox
-- MAGIC
-- MAGIC ## 3/ Build table to join sensor aggregates with wind turbine metadata and historical status
